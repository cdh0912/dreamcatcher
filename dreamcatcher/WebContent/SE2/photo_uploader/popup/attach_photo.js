	...
	var rFilter = /^(image\/bmp|image\/gif|image\/jpg|image\/jpeg|image\/png|image\/pdf)$/i;  
	var rFilter2 = /^(bmp|gif|jpg|jpeg|png|pdf)$/i; 
	var nTotalSize = 0;
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("이미지파일 (jpg,gif,png,bmp,pdf)만 업로드 가능합니다.");
    ...
    function html5Upload() {	
    	var tempFile,
    		sUploadURL;
    	/*sUploadURL = '/dreamcatcher/site?act=imageUpload'; */
/*    	//sUploadURL= 'file_uploader_html5.jsp'; 	//upload URL*/ 
    	sUploadURL = '/dreamcatcher/SE2/file_uploader_html5.jsp' //upload URL
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("이미지 파일(jpg,gif,png,bmp,pdf)만 업로드 하실 수 있습니다. ("+sFileName+")");
					}else{
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	    	...
 	    	/*sUrl : '/dreamcatcher/site?act=imageUpload',*/
 	    	sUrl : '/dreamcatcher/SE2/file_uploader.jsp',
 	    	sCallback : '/dreamcatcher/SE2/callback.html',
 			sFiletype : "*.jpg;*.png;*.bmp;*.gif;*.pdf",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP, PDF 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
