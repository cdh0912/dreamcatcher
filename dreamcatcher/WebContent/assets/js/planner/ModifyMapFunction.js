...
var LatLngList = new Array();//시작시 자동설정 위치 계산하기 위한 각 위치의 위경도 리스트
var bound = new google.maps.LatLngBounds();//시작시 자동 설정될 위치

var plumMarker = new google.maps.MarkerImage(root+"/assets/img/planner/dot_plum.png",
...
function addBox(sname, sid, lat, lng) {
	...
	$("#stop").attr("id", "stop" + idNum).append("<tr><td width='70%'>"+sname+"</td><td width='30%'>"
			+ "<img onclick='markerClick(" + sid + ")' src='"+root+"/assets/img/planner/stop_info.png' class='stop_infoImg'/>"
			+ "<img onclick='javascript:removeBox(this);' src='"+root+"/assets/img/planner/stop_remove.png' class='stop_removeImg'/></td></tr>");
...
function removeBox(element) {
	removeLine(); // 폴리라인 지우는 함수 호출.
	
	$(element).parent().parent().parent().parent().fadeTo("fast", 0.00, function(){ //fade
...
function LoadMap() {
	...
	map = new google.maps.Map(document.getElementById("map_canvas"), mapOptions);
		...
		//시작시 줌레벨, 센터를 LatLngList의 위경도로 자동설정
	for (var i = 0, LtLgLen = LatLngList.length; i < LtLgLen; i++) {
		bound.extend(LatLngList[i]); }
	map.fitBounds(bound);
	//--------------------------------- 지도 기본옵션 끝 ---------------------------------------	
...
//		}
		...
		(function(marker, site) {
			...
			if (site.savefolder != undefined || site.saved_picture != undefined) {
				siteImageUrl = realRoot + "/upload/" + site.savefolder + "/" + site.saved_picture;
			} else {
				siteImageUrl = root + "/assets/img/planner/NoImage.jpg";
			}
			...
			google.maps.event.addListener(marker, "click", function(e) {
				//인포윈도우 내용설정
					infoWindow.setContent(
						...
						"		<img src='"+root +"/assets/img/planner/info_mvsite.png' id='info_mvsitebtn'></a>													" +
						"		<a href=\"javascript:addBox('" + site.site_name + "','" + site.site_id + "','" + site.latitude + "','" + site.longitude + "');\">	" + 
						"		<img src='"+root +"/assets/img/planner/info_plus.png' id='info_plusbtn' onclick=''></a>												" +
						"	</div>																																	" +
						"</div>																																		"
					);
...
function makeRouteImageURL() {
	var latlngs = "";
	for(var i=0; i<polylist.length; i++) {
		latlngs += polylist[i].latitude + ",%20" + polylist[i].longitude;
		if(i < polylist.length-1) {
			latlngs += "|";
		}
	}
//	alert(latlng);												//사이즈 상수만들기
	route_url = "http://maps.googleapis.com/maps/api/staticmap?size=600x600&maptype=roadmap\%20&markers=size:mid|color:red|"
			+ latlngs // 마커 위치
			+ "&sensor=false&path=color:0xC90000bb|weight:7|"
			+ latlngs; // 경로 위치
